...
function getLang(str){
  ...
  return values[keys.findIndex(function(e){
    return e == str
  })]
}
function getLangArgs(str,args){
  keys = Object.keys(lang)
  values = Object.values(lang)
  let item = values[keys.findIndex(function(e){
    return e == str
  })]
  let command;
  args.forEach(function(f){
    if (f.with == undefined){
      item.replace("%s",f.text)
    }else{
      command = f.with.translate
      f.with.forEach(function(e){
        item.replace("%s",e)
      })
    }
  })
...
function testItemArgs(item,args){
  if (typeof item == "string"){
    return item
  }else{
    return getLangArgs(item,args)
  }
}
function testItem(item,args){
  if (typeof item == "string"){
    return item
  }else{
    return getLang(item)
